...
#really bad values
#model <- glmnet(data.matrix(data2[,-1,drop=FALSE]), as.matrix(data2[,1,drop=FALSE]), alpha = enet$finalModel$tuneValue$fraction, lambda=enet$finalModel$lambda, standardize = FALSE, family = "gaussian", type.measure="deviance")
...
best_caret <- train(f, data = data2,method = "leapBackward", trControl = train.control)
...
predict_glm <- predict.glmnet(model,data.matrix(set.test[,-1,drop=FALSE]),lambda=model$finalModel$tuneValue$lambda,s0=model$finalModel$tuneValue$fraction)
...
#predict_enet <- predict(model_enet,set.test[,-1])
